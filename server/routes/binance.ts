...
export const handleCreateUser: RequestHandler = async (req, res) => {
  ...
  try {
    ...
    const platformResponse = await fetch(`${DJANGO_API_BASE}/platforms/?name=Binance`);
    let platformData = await platformResponse.json();
    let platformId: number;

    if (platformData.length === 0) {
      // Create Binance platform if it doesn't exist
...
export const handleLogin: RequestHandler = async (req, res) => {
  ...
  try {
    ...
    const accountsResponse = await fetch(
      `${DJANGO_API_BASE}/accounts/?user=${user.id}&platform__name=Binance`
    );
    ...
    if (accounts.length === 0) {
      const response: ApiResponse = {
        success: false,
        error: "Binance account not found",
      };
      return res.status(404).json(response);
    }
...
export const handleSendCrypto: RequestHandler = async (req, res) => {
  ...
  try {
    ...
    const senderAccountsResponse = await fetch(
      `${DJANGO_API_BASE}/accounts/?user=${sender.id}&platform__name=Binance`
    );
    ...
    const recipientAccountsResponse = await fetch(
      `${DJANGO_API_BASE}/accounts/?user=${recipient.id}&platform__name=Binance`
    );
...
export const handleGetAllUsers: RequestHandler = async (req, res) => {
  try {
    ...
    if (!usersResponse.ok) {
      const response: ApiResponse = {
        success: false,
        error: usersData.error || "Failed to fetch Binance users",
      };
...
export const handleFundWallet: RequestHandler = async (req, res) => {
  ...
  try {
    ...
    // Get user account
    const accountsResponse = await fetch(
      `${DJANGO_API_BASE}/accounts/?user=${userId}&platform__name=Binance`
    );
    const accounts = await accountsResponse.json();
...
export const handleDeleteUser: RequestHandler = async (req, res) => {
  ...
  try {
    ...
    // Get user account
    const accountsResponse = await fetch(
      `${DJANGO_API_BASE}/accounts/?user=${id}&platform__name=Binance`
    );
    const accounts = await accountsResponse.json();
