...
import { User, Transaction, SendMoneyRequest, GCashSendMoneyRequest, FundWalletRequest, CreateUserRequest, LoginRequest, ApiResponse, SendMessageRequest, ChatMessage } from "@shared/api";
...
export const handleCreateUser: RequestHandler = async (req, res) => {
  ...
  try {
    // Call GCash-specific register endpoint which properly handles password hashing and platform association
    const userResponse = await fetch(`${DJANGO_API_BASE}/GCash/register`, {
      method: "POST",
      headers: {
...
export const handleLogin: RequestHandler = async (req, res) => {
  ...
  try {
    // Call GCash-specific login endpoint which properly verifies passwords
    const loginResponse = await fetch(`${DJANGO_API_BASE}/GCash/login`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
...
export const handleGetUserByEmail: RequestHandler = async (req, res) => {
  ...
  try {
    ...
    const accountsResponse = await fetch(
      `${DJANGO_API_BASE}/accounts/?user=${user.id}&platform__name=GCash`
    );
...
export const handleGetUserByMobile: RequestHandler = async (req, res) => {
  ...
  try {
    const response = await fetch(`${DJANGO_API_BASE}/GCash/user/mobile/${mobile}`);
...
export const handleSearchUsersByMobile: RequestHandler = async (req, res) => {
  ...
  try {
    const searchResponse = await fetch(`${DJANGO_API_BASE}/GCash/user/search?q=${encodeURIComponent(query)}`);
...
export const handleSendMoney: RequestHandler = async (req, res) => {
  const { recipientMobile, amount, message }: GCashSendMoneyRequest = req.body;
  const { senderMobile } = req.params;

  try {
    // Find sender by mobile number
    const senderResponse = await fetch(
      `${DJANGO_API_BASE}/GCash/user/mobile/${senderMobile}`
    );
    ...
    const recipientResponse = await fetch(
      `${DJANGO_API_BASE}/GCash/user/mobile/${recipientMobile}`
    );
    ...
    const senderAccountsResponse = await fetch(
      `${DJANGO_API_BASE}/accounts/?user=${sender.id}&platform__name=GCash`
    );
    ...
    const recipientAccountsResponse = await fetch(
      `${DJANGO_API_BASE}/accounts/?user=${recipient.id}&platform__name=GCash`
    );
...
export const handleGetTransactionsByMobile: RequestHandler = async (req, res) => {
  ...
  try {
    // Find user by mobile
    const userResponse = await fetch(`${DJANGO_API_BASE}/GCash/user/mobile/${userMobile}`);
    ...
    // Filter to get only GCash accounts for the specific user
    const GCashAccounts = accounts.filter((account: any) => 
      account.platform_name === "GCash" && account.user === parseInt(user.id)
    );

    if (GCashAccounts.length === 0) {
      const response: ApiResponse = {
        success: false,
        error: "GCash account not found for user",
      };
      return res.status(404).json(response);
    }

    const userAccount = GCashAccounts[0];
...
export const handleGetTransactions: RequestHandler = async (req, res) => {
  ...
  try {
    ...
    // Filter to get only GCash accounts for the specific user
    const GCashAccounts = accounts.filter((account: any) => 
      account.platform_name === "GCash" && account.user === user.id
    );
...
export const handleFundWallet: RequestHandler = async (req, res) => {
  ...
  try {
    // Get user account
    const accountsResponse = await fetch(
      `${DJANGO_API_BASE}/accounts/?user=${userId}&platform__name=GCash`
    );
    const accounts = await accountsResponse.json();
...
export const handleGetAllUsers: RequestHandler = async (req, res) => {
  try {
    // Call GCash-specific admin endpoint to get all GCash users
    const usersResponse = await fetch(`${DJANGO_API_BASE}/GCash/admin/users`);
...
export const handleGetAllTransactions: RequestHandler = async (req, res) => {
  try {
    // Call GCash-specific admin endpoint to get all transactions
    const transactionsResponse = await fetch(`${DJANGO_API_BASE}/GCash/admin/transactions`);
...
export const handleGetUserById: RequestHandler = async (req, res) => {
  ...
  try {
    const userResponse = await fetch(`${DJANGO_API_BASE}/GCash/admin/user/${id}`);
    const data = await userResponse.json();
...
export const handleUpdateUser: RequestHandler = async (req, res) => {
  ...
  try {
    const updateResponse = await fetch(`${DJANGO_API_BASE}/GCash/admin/user/${id}/update`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
      },
...
export const handleAdjustUserBalance: RequestHandler = async (req, res) => {
  ...
  try {
    const adjustResponse = await fetch(`${DJANGO_API_BASE}/GCash/admin/adjust-balance`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
...
export const handleSendMessage: RequestHandler = async (req, res) => {
  ...
  try {
    const response = await fetch(`${DJANGO_API_BASE}/GCash/chat/send`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
...
export const handleMarkMessagesRead: RequestHandler = async (req, res) => {
  ...
  try {
    const response = await fetch(`${DJANGO_API_BASE}/GCash/chat/mark-read/${email}`, {
      method: "POST",
    });
...
export const handleGetChatHistory: RequestHandler = async (req, res) => {
  ...
  try {
    const response = await fetch(`${DJANGO_API_BASE}/GCash/chat/history/${email}`);
    const data = await response.json();
...
export const handleGetUnreadMessages: RequestHandler = async (req, res) => {
  ...
  try {
    const response = await fetch(`${DJANGO_API_BASE}/GCash/chat/unread/${email}`);
    const data = await response.json();
...
export const handleGetChatStatus: RequestHandler = async (req, res) => {
  ...
  try {
    const response = await fetch(`${DJANGO_API_BASE}/GCash/chat/status/${email}`);
    const data = await response.json();
...
export const handleToggleChat: RequestHandler = async (req, res) => {
  ...
  try {
    const response = await fetch(`${DJANGO_API_BASE}/GCash/admin/user/${userId}/toggle-chat`, {
      method: "POST",
    });
