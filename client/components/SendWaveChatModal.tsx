...
const SendWaveChatModal: React.FC<ChatModalProps> = ({ isOpen, onClose, user, onMarkAsRead }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg w-full max-w-sm sm:max-w-md md:max-w-lg h-[500px] sm:h-[600px] flex flex-col">
        {/* Modal Header */}
        <div className="bg-[#007DFE] text-white p-4 rounded-t-lg flex justify-between items-center">
          <div>
          ...
          {messages.length === 0 ? (
            ...
            messages.map((message) => (
              <div
                key={message.id}
                className={`flex ${
                  message.sender_email === "admin@sendwave.com" ? "justify-end" : "justify-start"
                }`}
              >
                <div
                  className={`max-w-[70%] p-3 rounded-lg ${
                    message.sender_email === "admin@sendwave.com"
        ...
        <div className="p-4 border-t">
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#007DFE]"
              disabled={loading}
            />
            <button
              onClick={sendMessage}
              disabled={loading || !newMessage.trim()}
              className="bg-[#007DFE] text-white px-4 py-2 rounded-lg hover:bg-blue-700 disabled:opacity-50 transition-colors"
            >
              {loading ? (
...
export default SendWaveChatModal;
