...
const CryptoPortChatWidget: React.FC<ChatWidgetProps> = ({ user, isOpen, onClose }) => {
  ...
  const fetchChatHistory = async () => {
    try {
      const response = await fetch(`/api/binance/chat/history/${user.email}/`);
  ...
  const markMessagesRead = async () => {
    try {
      await fetch(`/api/binance/chat/mark-read/${user.email}/`, {
        method: "POST",
      });
    } catch (error) {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/binance/chat/send/", {
        method: "POST",
        headers: {
    ...
    return (
      <div className="fixed bottom-4 right-4">
        <button
          ...
          className="bg-yellow-500 text-black p-4 rounded-full shadow-lg hover:bg-yellow-600 transition-colors"
        >
  ...
  return (
    <div className="fixed bottom-4 right-4 w-full max-w-xs sm:w-80 h-[500px] sm:h-96 bg-gray-800 border border-gray-700 rounded-lg shadow-lg flex flex-col">
      {/* Chat Header */}
...
export default CryptoPortChatWidget;
