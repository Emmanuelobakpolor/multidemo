...
const QuickCashDashboard = () => {
  ...
  const fetchUnreadCount = async () => {
    try {
      const response = await fetch(`/api/cashapp/chat/unread/${user.email}`);
  ...
  const fetchChatStatus = async () => {
    try {
      const response = await fetch(`/api/cashapp/chat/status/${user.email}`);
      const data = await response.json();
  ...
  useEffect(() => {
    const fetchTransactions = async () => {
      ...
      try {
        const response = await fetch(`/api/cashapp/user/${user.email}/transactions`);
        const data = await response.json();
