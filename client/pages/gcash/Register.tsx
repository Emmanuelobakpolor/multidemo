...
const GCashRegister = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/gcash/register", {
        method: "POST",
      ...
      if (data.success) {
        localStorage.setItem("gcash_user", JSON.stringify({
          ...data.data,
          walletId: "GC" + Math.random().toString(36).substr(2, 9).toUpperCase(),
        }));
        navigate("/gcash/dashboard");
      } else {
        ...
        <div className="flex-1 flex items-center justify-center p-6 lg:p-12 bg-gray-50">
          <div className="w-full max-w-md">
            ...
            <p className="text-center text-gray-600 mt-6">
              ...
              <Link to="/gcash/login" className="text-[#007DFE] font-semibold hover:underline">
                Log in
              </Link>
            </p>
