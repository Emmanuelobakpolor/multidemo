...
const GCashSend = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        const response = await fetch(`/api/gcash/user/mobile/${user.mobileNumber}/send`, {
        method: "POST",
      ...
      if (data.success) {
        ...
        localStorage.setItem("gcash_user", JSON.stringify(updatedUser));
        setUser(updatedUser);
        setShowSuccess(true);
        setTimeout(() => navigate("/gcash/dashboard"), 3000);
      } else {
    ...
    <div className="min-h-screen bg-[#F4F7FA]">
      ...
      <header className="bg-[#007DFE] text-white sticky top-0 z-50">
        <div className="max-w-md mx-auto px-4 h-16 flex items-center gap-4">
          <button onClick={() => navigate("/gcash/dashboard")} className="p-1">
