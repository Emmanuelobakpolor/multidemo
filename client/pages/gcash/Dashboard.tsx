...
const SendWaveDashboard = () => {
  ...
  const fetchTransactions = async () => {
    try {
      const response = await fetch(`/api/gcash/user/mobile/${user.mobileNumber}/transactions`);
  ...
  const checkChatStatus = async () => {
    try {
      const response = await fetch(`/api/gcash/chat/status/${user.email}`);
      const data = await response.json();
      if (data.success) {
    ...
    <div className="min-h-screen bg-[#F4F7FA]">
      ...
      <main className="max-w-md mx-auto pb-20">
        ...
        <div className="grid grid-cols-4 gap-4 px-4 mt-8">
          <Link to="/gcash/send" className="flex flex-col items-center gap-2">
          ...
          <Link to="/gcash/receive" className="flex flex-col items-center gap-2">
            <div className="w-14 h-14 bg-white rounded-2xl shadow-sm flex items-center justify-center text-[#007DFE]">
