...
const PayFlowSend = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/paypal/user/${user.email}/send`, {
        method: "POST",
        headers: {
      ...
      if (data.success) {
        ...
        localStorage.setItem("paypal_user", JSON.stringify(updatedUser));
        setUser(updatedUser);
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center px-4">
        <div className="text-center max-w-md">
          ...
          <h1 className="text-2xl font-semibold text-gray-900 mb-2">Money sent</h1>
          <p className="text-gray-600 mb-1">
            You sent ${parseFloat(formData.amount).toFixed(2)} USD
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
              <p className="text-sm text-gray-700">
                <span className="font-medium">Sending to a friend?</span> Make sure you trust this person. 
                You're not covered by PayPal Purchase Protection for friends and family payments.
              </p>
            </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3 bg-blue-600 text-white rounded-full font-medium hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-sm"
            >
