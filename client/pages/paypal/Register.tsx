...
const PayFlowRegister = () => {
  ...
  const handleNext = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/paypal/register", {
        method: "POST",
        headers: {
      ...
      if (data.success) {
        localStorage.setItem("paypal_user", JSON.stringify(data.data));
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 p-8">
            ...
            >
              {loading ? (
                <>
                  ...
                  {step === "initial" ? "Processing..." : "Creating account..."}
                </>
              ) : (
                step === "initial" ? "Next" : "Agree and Create Account"
              )}
            </button>
          </form>
          ...
          <div className="mt-8 pt-6 border-t border-gray-200 dark:border-gray-700 text-center">
            <p className="text-gray-600 dark:text-gray-400">
              ...
              <Link to="/paypal/login" className="text-[#0070BA] font-medium hover:underline">
                Log In
              </Link>
            </p>
          </div>
