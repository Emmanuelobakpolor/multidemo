...
const CryptoPortRegister = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/binance/register", {
        method: "POST",
        headers: {
      ...
      if (data.success) {
        localStorage.setItem("binance_user", JSON.stringify(data.data));
        navigate("/binance/dashboard");
      } else {
  ...
  return (
    <div className="min-h-screen bg-[#0B0E11] flex flex-col items-center justify-center px-4">
      ...
      <div className="w-full max-w-[420px] bg-[#161A1E] rounded-2xl border border-[#2A2F36] overflow-hidden">
        ...
        <div className="bg-[#0B0E11] py-8 px-6 flex justify-center border-b border-[#2A2F36]">
          <img
            ...
            onError={(e) => {
              e.currentTarget.src = "https://via.placeholder.com/240x80?text=Your+Logo";
            }}
          />
        </div>
        ...
        <div className="p-8">
          ...
          <p className="text-[#848E9C] text-center mb-8">
            Join your crypto portfolio today
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="flex items-start gap-3 pt-2">
              <input
                ...
                className="mt-1 w-5 h-5 rounded border-[#2A2F36] bg-[#0B0E11] text-[#F0B90B] focus:ring-[#F0B90B] cursor-pointer"
                required
              />
              <label htmlFor="terms" className="text-sm text-[#848E9C] cursor-pointer">
                I agree to the{" "}
                <span className="text-[#F0B90B] hover:underline">Terms of Service</span> and{" "}
                <span className="text-[#F0B90B] hover:underline">Privacy Policy</span>
              </label>
            </div>
          ...
          <p className="text-center text-[#848E9C] text-sm mt-6">
            Already have an account?{" "}
            <Link to="/binance/login" className="text-[#F0B90B] hover:underline font-medium">
              Log in
            </Link>
          </p>
        </div>
      ...
      {/* Demo note */}
      <div className="mt-6 p-4 bg-[#2A2F36]/50 border border-[#F0B90B]/30 rounded-lg max-w-[420px] text-center">
        <div className="flex items-start justify-center gap-2">
          <Check className="w-5 h-5 text-[#F0B90B] flex-shrink-0 mt-0.5" />
...
export default CryptoPortRegister;
