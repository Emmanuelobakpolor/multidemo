...
const BinanceSend = () => {
  ...
  const fetchCryptoWallets = async (email: string) => {
    try {
      const response = await fetch(`/api/binance/user/${email}/wallets`);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`/api/binance/user/${user.email}/send`, {
        method: "POST",
        headers: {
      ...
      <div className="min-h-screen bg-[#0B0E11]" style={{ fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif' }}>
        ...
        <div className="flex items-center justify-center py-16 px-4">
          <div className="max-w-md w-full">
            ...
            <button
              onClick={() => navigate("/binance/dashboard")}
