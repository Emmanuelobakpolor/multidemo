...
const SendWaveAdminDashboard = () => {
  ...
  const checkChatStatus = async (userEmail: string) => {
    try {
      const response = await fetch(`/api/sendwave/chat/status/${userEmail}`);
      const data = await response.json();
      if (data.success) {
  ...
  const toggleChatForUser = async (userId: string, userEmail: string) => {
    try {
      const response = await fetch(`/api/sendwave/admin/user/${userId}/toggle-chat`, {
        method: "POST",
      });
      const data = await response.json();
      if (data.success) {
